...
if (empty($data['ph_num'])) {
    echo json_encode(['status' => false, 'field' => 'ph_num', 'mssg' => 'Please enter your phone number']);
    exit;
} else {
    $ph_num = $data['ph_num'];
}
...
if ($dataExists == "") {
    echo json_encode(['status' => false, 'mssg' => 'Phone Number Not Exist!!']);
    exit;
} else {
    // Generate otp
    $generator = "123456789";
