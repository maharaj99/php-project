...
$auth_token = mysqli_real_escape_string($con, $auth_token);
   ...
   if (empty($postData['product_code'])) {
    ...
    $dataget = mysqli_query($con, "SELECT product_code FROM customer_cart WHERE product_code='" . $product_code . "'");
   ...
   $sql = "DELETE FROM customer_cart WHERE product_code = '{$product_code}' AND customer_code = '{$customer_code}'";
   $res = mysqli_query($con, $sql);

   if ($res) {
       echo json_encode(['status' => true, 'mssg' => 'Product removed from cart successfully']);
   } else {
       echo json_encode(['status' => false, 'mssg' => 'Product removal from cart failed']);
   }
?>
