<?php
   ...
   $dataget = mysqli_query($con, "SELECT * FROM customer_wishlist WHERE product_code='" . $product_code . "' AND customer_code='" . $customer_code . "'");
   $dataExists = mysqli_fetch_assoc($dataget);
   if($dataExists!=""){
    echo json_encode(['status'=>false,'mssg'=>'Already Wish Listed this product']);
    exit;

   }
   
   $sql="insert into customer_wishlist(whishlist_code,customer_code,product_code,entry_user_code) values ('{$whishlist_code}','{$customer_code}','{$product_code}','{$customer_code}')";
   $res=mysqli_query($con,$sql);   

   if($res)
   {

       echo json_encode(['status'=>true,'mssg'=>'Product Wish listed swucessfully']);

   }
   else
   {
       echo json_encode(['status'=>false,'mssg'=>'Product Wish listed failed']);
   }
?>